...
from .tools import reflect_answer, save_markdown_file
...
def run_week3_reflection_agent(
    ...
    # Import tools from your registry
    from .tools import arxiv_search_papers, search_tool, reflect_answer, save_markdown_file
    ...
    while True:
        ...
        if ai.tool_calls:
            ...
            # After tools, force a written answer
            draft = _force_write_answer(llm, messages, query)
        else:
            # No tools called; still force answer if content is empty
            draft = _clean_text(ai.content) or _force_write_answer(llm, messages, query)
        ...
        if next_action == "search_more":
            ...
            messages.append(HumanMessage(content=f"Do more research with tools. {hint} Then answer again."))
        else:
            issues = reflection.get("issues", [])
            messages.append(HumanMessage(content=f"Rewrite and fix: {issues}. Follow the required format."))
    ...
    return {"final": best_draft, "steps": steps, "retries": retries}
